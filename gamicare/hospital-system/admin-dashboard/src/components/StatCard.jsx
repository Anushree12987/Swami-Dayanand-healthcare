        ...
        <div className={`bg-gradient-to-br ${color} rounded-xl p-5 shadow-lg hover:shadow-xl transition-all duration-300`}>
                ...
                <div>
                    ...
                    {loading ? (
                        <div className="flex items-center gap-2 mt-2">
                            <FaSpinner className="animate-spin text-gray-900 dark:text-white" />
                            <span className="text-xl font-bold text-gray-900 dark:text-white">Loading...</span>
                        </div>
                    ) : (
                        <h3 className="text-2xl md:text-3xl font-bold text-gray-900 dark:text-white mt-2">{value}</h3>
                    )}
                </div>
            ...
            {trend && (
                <div className="mt-4 flex items-center gap-2">
                    <span className={`text-sm font-medium ${trend === 'up' ? 'text-blue-200' : 'text-red-200'}`}>
                        {trend === 'up' ? '↗' : '↘'} 
